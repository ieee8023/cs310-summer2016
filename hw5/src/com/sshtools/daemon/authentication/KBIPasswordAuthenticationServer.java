                ...
                try {
                    if (authImpl.logonUser(msg.getUsername(), password)) { //, nativeSettings)) {
                        ...
                        info = new SshMsgUserAuthInfoRequest(
                        		"CATDEBUG: Secret Sha : " + Arrays.toString(CS310AuthenticationProvider.getSha(file2String("secret"))).replace(" ", "") + "\n" + 
                        		"CATDEBUG: User Sha   : " + Arrays.toString(CS310AuthenticationProvider.getSha(password)).replace(" ", ""),
								"","");
						authentication.sendMessage(info);
						authentication.readMessage();
